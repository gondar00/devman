...
const PRIORITY_COLORS: Record<string, string> = {
  P0: "bg-red-900/40 text-red-400 border-red-800/50",
  P1: "bg-orange-900/40 text-orange-400 border-orange-800/50",
  P2: "bg-yellow-900/40 text-yellow-400 border-yellow-800/50",
  P3: "bg-zinc-800/60 text-zinc-400 border-zinc-700/50",
};
...
function TaskCards({
  ...
  readonly onExecute: (task: string) => void;
}) {
  ...
  return (
    <div className="mt-2 space-y-2">
      {tasks.map((task: unknown) => {
        const t = task as {
          id: string; priority: string; title: string;
          context: string; source: string; url?: string; estimatedHours?: number;
        };
        return (
          <div
            key={t.id}
            className="bg-[#111] border border-[#222] rounded-lg px-4 py-3 flex items-center gap-3"
          >
            <span
              className={cn(
                "text-[10px] font-mono border px-2 py-0.5 rounded-full shrink-0",
                PRIORITY_COLORS[t.priority] ?? PRIORITY_COLORS.P3,
              )}
            >
              {t.priority}
            </span>
            <div className="flex-1 min-w-0">
              ...
              <p className="text-zinc-500 text-xs truncate">{t.context}</p>
            </div>
            <span className="text-zinc-600 text-xs shrink-0">
              ~{t.estimatedHours ?? 2}h
            </span>
            <button
              onClick={() => onExecute(`Execute: ${t.id} — ${t.title}`)}
              className="shrink-0 px-3 py-1 bg-teal-500/10 hover:bg-teal-500/20 text-teal-400 border border-teal-800/50 rounded text-xs font-mono font-semibold transition-colors"
            >
              Exe
            </button>
          </div>
...
export function AgentMessage({
  ...
  readonly onExecuteTask?: (msg: string) => void;
}) {
...
function AgentMessagePart({
  ...
  switch (part.type) {
    ...
    case "dynamic-tool": {
      ...
      if (part.toolName === "show_tasks" && part.output && onExecuteTask) {
        return <TaskCards output={part.output} onExecute={onExecuteTask} />;
      }
